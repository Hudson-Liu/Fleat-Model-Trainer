...
nlp_features = 1 #only 1 feature, being the individual character
...
anet.add(keras.layers.LSTM(units = nlp_embedding, activation = "relu", stateful = True, batch_input_shape = (batch_size, None, nlp_embedding)))
...
hnet.add(keras.layers.Dense(units = nlp_embedding))
...
def rand_lstm_weights(lstm_nodes, n_dense):
    ...
    W = [[(20 * random.random() - 10) for i in range(0, lstm_nodes * 4)]] #weights can be any value
    ...
    for layer in n_dense:
        ...
        if i != 0:
            ...
            weights = np.array([[(20 * random.random() - 10) for i in range(0, layer)] for l in range(0, lstm_nodes)], dtype=float)
        wb.append(weights)
...
nlp_wb = rand_lstm_weights(lstm_nodes = nlp_nodes, n_dense = [nlp_dense, nlp_embedding])
nlp.set_weights(nlp_wb)
anet_wb = rand_lstm_weights(lstm_nodes = nlp_embedding, n_dense = [anet_dense, anet_pred_vars])
anet.set_weights(anet_wb)
hnet_wb = rand_lstm_weights(lstm_nodes = nlp_embedding, n_dense = [hnet_dense, hnet_pred_vars])
